...
export type DirectMailCampaign = CallCampaign & {
  template: { id: string; name: string };
  mailType: "postcard" | "letter" | "self_mailer";
  mailSize: "4x6" | "6x9" | "8.5x11";
...
export function generateDirectMailCampaignData(): DirectMailCampaign[] {
  ...
  return base.map((r, i) => {
    ...
    const leadCount = Math.max(1, Math.min(8, (r.leads ?? Math.floor((r.calls ?? 5) / 2)) || 3));
    const leadsDetails: DirectMailLead[] = Array.from({ length: leadCount }).map((_, idx) => {
      ...
      const mailings: DirectMailMailing[] = Array.from({ length: mailingCount }).map((__, m) => {
        const chosenTmpl = templates[(i + idx + m) % templates.length]!;
        const chosenType = mailTypes[(i + idx + m) % mailTypes.length] as DirectMailMailing["mailType"];
        const chosenSize = mailSizes[(i + m) % mailSizes.length] as DirectMailMailing["mailSize"];
        const sendOffset = ((m + 1) * ((idx % 5) + 1)) * day; // spaced apart
        const sendDate = new Date(now - sendOffset).toISOString();
        const expectedDeliveryAt = new Date(new Date(sendDate).getTime() + (2 + (m % 5)) * day).toISOString();
        const status: DirectMailMailing["status"] = m === mailingCount - 1 && (i + idx) % 4 === 0
          ? "in_transit"
          : (m + i + idx) % 7 === 0
          ? "returned"
          : (m + i) % 9 === 0
        ...
        const transfers = (idx + m) % 4 === 0 ? Math.floor(Math.random() * 3) : 0;
        return {
          id: `${id}_ml_${m + 1}`,
    ...
    const lob: DirectMailLobInfo = isLetter
      ...
      ? {
          ...
          expected_delivery_date: new Date(now + ((i % 10) + 2) * day).toISOString().slice(0, 10),
          color: false,
          tracking_events: [],
          to: address,
          from: fromAddress,
