...
function collectLeadsByList(rows: DemoRow[]): Map<string, DemoLead[]> {
  ...
  for (const r of rows) {
    ...
    const leads: DemoLead[] = Array.isArray(maybe.leads) ? maybe.leads as DemoLead[] : [];
...
export async function exportNestedLeadsToExcelZipPerList(rows: DemoRow[], zipName = "nested-leads-per-list") {
...
export async function exportNestedLeadsToExcel(rows: DemoRow[], filenameBase = "nested-leads") {
  const groups = collectLeadsByList(rows);
  if (groups.size === 0) return;
  let ExcelModule: typeof import("exceljs");
  try {
  ...
  for (const [listName, leads] of groups.entries()) {
    for (const lead of leads) {
      const flat = leadToFlatRow(lead) as Record<string, unknown>;
      ws.addRow([listName, ...leadHeaderOrder.map((h) => String(flat[h] ?? ""))]);
    }
  }
  const buf = await wb.xlsx.writeBuffer();
...
export async function exportNestedLeadsToCSVZipPerList(rows: DemoRow[], zipName = "nested-leads-per-list-csv") {
  const groups = collectLeadsByList(rows);
  if (groups.size === 0) return;
  let JSZipClass: typeof import("jszip");
  try {
  ...
  for (const [listName, leads] of groups.entries()) {
    const lines = leads.map((lead) => {
      const flat = leadToFlatRow(lead) as Record<string, unknown>;
      return [listName, ...leadHeaderOrder.map((h) => String(flat[h] ?? ""))].join(",");
    });
...
export function exportNestedLeadsToCSV(rows: DemoRow[], filenameBase = "nested-leads") {
  const groups = collectLeadsByList(rows);
  if (groups.size === 0) return;
  const headerLine = ["list", ...leadHeaderOrder].join(",");
  const lines: string[] = [];
  for (const [listName, leads] of groups.entries()) {
    for (const lead of leads) {
      const flat = leadToFlatRow(lead) as Record<string, unknown>;
      lines.push([listName, ...leadHeaderOrder.map((h) => String(flat[h] ?? ""))].join(","));
    }
  }
  const csv = [headerLine, ...lines].join("\n");
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
...
export async function exportRowsToCSVZipPerList(
  ...
  for (const [listName, listRows] of groups.entries()) {
    const lines = listRows.map((r) => headers.map((h) => String(r[h] ?? "")).join(","));
    const csv = [headerLine, ...lines].join("\n");
    const safe = listName.replace(/[^a-z0-9\-_]+/gi, "_");
    zip.file(`${safe}.csv`, csv);
  }
