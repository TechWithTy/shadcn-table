...
export function TopActionsBar({
  ...
  const openSkipTraceFromSelection = () => {
    ...
    const availableLeadCount = sourceRows.reduce((sum, r) => sum + getRowLeadCount(r), 0);
    const listCounts: Record<string, number> = sourceRows.reduce((acc, r) => {
      acc[r.list] = (acc[r.list] ?? 0) + getRowLeadCount(r);
      return acc;
    }, {} as Record<string, number>);
    const availableLists = lists.map((name) => ({ name, count: listCounts[name] ?? 0 }));
  ...
  return (
    <div className="ml-auto flex flex-wrap items-center gap-2">
      ...
      <Button variant="outline" size="sm" type="button" onClick={() => table.resetRowSelection()}>
        Clear
      </Button>
