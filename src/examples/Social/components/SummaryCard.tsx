...
// Minimal local types to model the interactions used by the summary
...
type SocialRowWithInteractions = CallCampaign & {
...
export const SummaryCard: React.FC<SummaryCardProps> = ({
	...
	const timeline = React.useMemo(() => {
		...
		for (const r of filtered as Array<SocialRowWithInteractions>) {
			const arr = r?.interactionsDetails;
			if (!arr || arr.length === 0) continue;
			for (const it of arr) {
				if (!it?.createdAt) continue;
				const label = `${it.type ?? "interaction"} by @${it.user ?? "user"}`;
				items.push({
					id: String(it.id ?? `${r.name}-${it.createdAt}`),
